object frmMdi2: TfrmMdi2
  ...
  object master: TADOQuery
    ...
    SQL.Strings = (
        ...
        'SELECT so.id_so,cust.namaperusahaan,so.kode,so.tgl_trans,so.Tota' +
        'lQty,so.TotalKg FROM tblso so LEFT JOIN tblpelanggan cust ON (so' +
    ...
    object masternamaperusahaan: TStringField
  ...
  object detail: TADOQuery
    ...
    Parameters = <>
    SQL.Strings = (
      'SELECT sd.kg,sd.qty,sd.setting,sd.jasapo FROM tblsodet sd')
    Left = 561
    Top = 321
